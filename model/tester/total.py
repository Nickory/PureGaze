...
def main(train, test):
    ...
    try:
        ...
        print(f"Error importing reader module: {e}")
        return
        ...
        with torch.no_grad():
            for j, (data, label) in enumerate(dataset):
                ...
                try:
                    ...
                    # Calculate error between each pair of result and gt ------------------
                    for k, result in enumerate(results):
                        result = result.cpu().detach().numpy()
                        gt = gts[k].cpu().numpy()

                        accs += gtools.angular(gtools.gazeto3d(gt), gazeto3d(result))
                    ...
                    continue  # Skip this sample and proceed with the next
